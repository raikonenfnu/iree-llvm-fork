...
// tensor.pad ops to allow handle padding-elided and padding-needed cases
// separately.
//
//===----------------------------------------------------------------------===//
...
/// Splits a tensor.pad op by wrapping it in a scf.if op to handle
...
struct TensorSplitPaddingPass final
    : public TensorSplitPaddingBase<TensorSplitPaddingPass> {
...
std::unique_ptr<Pass> mlir::createTensorSplitPaddingPass() {
  return std::make_unique<TensorSplitPaddingPass>();
}
